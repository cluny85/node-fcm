var pushTemplates = "./exampleTemplates/",
    pushTypes = require(pushTemplates),
    utils     = require("./push/push.utils");
const { EVENTS, FCM_RESPONSES } = require("./push/push.const");
// const EVENTS  = require("./push/push.const").EVENTS,
//       FCM_RESPONSES = require("./push/push.const").FCM_RESPONSES;
 ...
 * @param {*} fcmConfig Object with fcm params: cert & url
 * @param {*} pathTemplates Optional. Change the default push templates.
 */
module.exports = (fcmConfig,pathTemplates) => {
    if(!fcmConfig) throw new Error("Must provide an object with the firebase initialization.");
    const push = require("./lib/firebase")(fcmConfig);
     ...
     */
    function send(tokens, type, data){
        ...
        try{
            ...
            return safeSend(tokens, type, data);
        }
        catch(err){
    ...
    async function safeSend(tokens, type, data){
        var index = 0;
        var result = {tokens: 0, success: 0, failures: 0};
        ...
        do {
            ...
            for (var i = 0; i < so.length; i++) {
                if(tokensToSend[so[i]]){
                    let response = await push.sendToDevice(
                        tokensToSend[so[i]], 
